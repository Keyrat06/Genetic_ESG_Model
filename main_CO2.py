...
class GeneticPlayer:
    def __init__(self, capacity, var_cost, OM, num_periods=12, name="",
                 names=[], CO2=[], CO2_permit=0, r=10, s=1):
        self.name = name
        self.names = names
        ...
        self.OM = OM
        self.CO2 = CO2
        self.CO2_permit = CO2_permit
        self.r = r
        self.s = s
        self.bids = (1+self.r*np.random.random((self.N, num_periods))) * (self.MC[:, np.newaxis]) \
                                  + np.random.normal(0, self.s, (self.N, self.num_periods))**2
...
def play_game(players):
    ...
    total_CO2 = np.zeros(NUM_PLAYERS)
    for period in range(NUM_PERIODS):
        bids_quantities_MC_CO2_play_numbers = []
        for i, player in enumerate(players):
            for j, bid in enumerate(player.get_bids(period)):
                quantity = player.capacity[j]
                MC = player.MC[j]
                CO2 = player.CO2[j]
                bids_quantities_MC_CO2_play_numbers.append((bid, quantity, MC, CO2, i))
        bids_quantities_MC_CO2_play_numbers.sort()
        total_quantity = 0
        i = 0
        demand_this_period = demands[period]
        while total_quantity < demand_this_period:
            total_quantity += bids_quantities_MC_CO2_play_numbers[i][1]
            i += 1
        price = bids_quantities_MC_CO2_play_numbers[i-1][0]
        total_demand_remaining = demand_this_period
        for j in range(i):
            bid, quantity, MC, CO2, player_number = bids_quantities_MC_CO2_play_numbers[j]
            assert total_demand_remaining >= 0
            assert bid <= price
            assert MC <= bid
            total_CO2[player_number] += min(total_demand_remaining, quantity) * CO2
    ...
    for i, player in enumerate(players):
        total_profit[i] -= 100 * max((total_CO2[i] - player.CO2_permit, 0))
...
def mix_players(player_1, player_2):
    new_player = GeneticPlayer(player_1.capacity, player_1.MC, player_1.OM,
                               ...
                               player_1.CO2, player_1.CO2_permit, player_1.r, player_2.s)
...
def get_pop_types():
    ...
    for i in range(7):
        name = types[i]
        OM = sum(b[i].OM)
        CO2_permit = float((b[i].CO2_permit[0]))
        capacity = tuple(b[i].Capacity)
        var_cost = tuple(b[i].Total_Var_Cost)
        names = tuple(b[i].Parts)
        CO2 = tuple(np.array(b[i].CO2)/2000.0)
        data.append({"name": name, "OM": OM, "capacity": capacity, "var_cost": var_cost,
                     "names": names, "CO2": CO2, "CO2_permit": CO2_permit})
    return data
